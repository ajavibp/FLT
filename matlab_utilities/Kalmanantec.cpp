...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Generates outputs
	int numInMF = 0;
	mxArray *numOutMF = mxCreateDoubleMatrix(1,Model.outputs(),mxREAL);
	for (int j=0;j<Model.outputs();j++)
	{
		numInMF += Model.rules(j);
		*(mxGetPr(numOutMF)+j) = Model.rules(j);
	}

	mxArray *parameters[12];
	parameters[0] = mxCreateString("NumInputs");
	parameters[1] = mxCreateDoubleScalar(Model.inputs());
	parameters[2] = mxCreateString("NumInputMFs");
	parameters[3] = mxCreateDoubleScalar(numInMF);
	parameters[4] = mxCreateString("NumOutputs");
	parameters[5] = mxCreateDoubleScalar(Model.outputs());
